...
it('lets clinic staff schedule and rejects super_admin', function (string $role, bool $allowed) {
    $actor = User::factory()->state(['role' => $role])->create(['clinic_id' => $this->clinic->id]);
...
it('blocks super_admin from creating', function () {
    $superAdmin = User::factory()->superAdmin()->create(['clinic_id' => $this->clinic->id]);
    $start = now()->addDays(2)->setTime(10, 0);
